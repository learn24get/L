...
CMDs[#CMDs + 1] = {NAME = 'replicationlag [num]', DESC = 'Set IncommingReplicationLag'}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'r', DESC = 'Resets your character normally'}
...
CMDs[#CMDs + 1] = {NAME = 'steal [plr]', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'decalsteal [plr]', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'nosit', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'to [plr]', DESC = 'teleport to a player'}
CMDs[#CMDs + 1] = {NAME = 'get [shotty/uzi/sawed/cash]', DESC = 'teleport you to tools'}
CMDs[#CMDs + 1] = {NAME = 'gravgun', DESC = 'gives u a gravity gun'}
CMDs[#CMDs + 1] = {NAME = 'goto [van, inter, bank, beach, prison, court]', DESC = 'brings u to a place'}
CMDs[#CMDs + 1] = {NAME = 'nogh', DESC = 'turns on no ground hit'}
CMDs[#CMDs + 1] = {NAME = 'roll', DESC = 'allows u to roll'}
CMDs[#CMDs + 1] = {NAME = 'orbit [plr]', DESC = 'orbits around a player'}
CMDs[#CMDs + 1] = {NAME = 'unorbit / unorb', DESC = 'unorbits you'}
CMDs[#CMDs + 1] = {NAME = 'kneel', DESC = 'kneel idle animation'}
CMDs[#CMDs + 1] = {NAME = 'normal ', DESC = 'normal idle animation'}
CMDs[#CMDs + 1] = {NAME = 'ghost', DESC = 'ghost-like animation'}
CMDs[#CMDs + 1] = {NAME = 'find [plr]', DESC = 'finds a player'}
CMDs[#CMDs + 1] = {NAME = 'unfind [plr]', DESC = 'unfinds a player'}
CMDs[#CMDs + 1] = {NAME = 'annoy [plr]', DESC = 'annoys a player'}
CMDs[#CMDs + 1] = {NAME = 'unannoy', DESC = 'unannoys a player'}
CMDs[#CMDs + 1] = {NAME = 'instareset/ins', DESC = 'turns on instant reset'}
CMDs[#CMDs + 1] = {NAME = 'noinstantreset/noins', DESC = 'turns off instant reset'}
wait()
...
local PlaceTable = {
	['inter'] = CFrame.new(-310.058258, 3, -177.354538);
	...
	['van'] = CFrame.new(-136.972229, 3.4000001, -513.670654);
	['burg'] = CFrame.new(-724.581665, 4.33005905, 124.40332);
	['chick'] = CFrame.new(-742.330078, 4.39626884, -43.4574966);
	['m1'] = CFrame.new(-403.653625, 103.5, -178.768326);
}
...
function(args, speaker)
	...
	if speaker.Character then
		for i,_ in pairs(WayPoints) do
			...
			local z = WayPoints[i].COORD[3]
			if tostring(WayPoints[i].NAME):lower() == tostring(WPName):lower() then
				game:GetService("TweenService"):Create(speaker.Character.HumanoidRootPart, TweenInfo.new(1, Enum.EasingStyle.Linear), {CFrame = CFrame.new(x,y,z)}):Play()
			end
		end
		for i,_ in pairs(pWayPoints) do
			if tostring(pWayPoints[i].NAME):lower() == tostring(WPName):lower() then
				game:GetService("TweenService"):Create(speaker.Character.HumanoidRootPart, TweenInfo.new(1, Enum.EasingStyle.Linear), {CFrame = CFrame.new(pWayPoints[i].COORD[1].Position)}):Play()
			end
		end
	end
end)
...
function(args, speaker)
	if gethidden then
		...
		for i,v in pairs(players) do
			local p = Players[v]
			table.insert(regions,p.Name.."'s region is: "..gethidden(p, "CountryRegionCodeReplicate"))
		end
...
function(args, speaker)
	if sethidden then	
		sethidden(speaker, "CountryRegionCodeReplicate", getstring(1))
...
addcmd('stun',{},
function(args, speaker)
	speaker.Character.Humanoid:ChangeState(16)
end)

addcmd('unstun',{},
function(args, speaker)
	speaker.Character.Humanoid:ChangeState(14)
end)
...
function(args, speaker)
	...
	invisflingStepped = game:GetService('RunService').Stepped:connect(function()
		if speaker.Character and speaker.Character:FindFirstChildOfClass("HumanoidRootPart") then
			speaker.Character.HumanoidRootPart.CanCollide = false
		else
	...
	bambam.Location = speaker.Character.HumanoidRootPart.Position
end)
		...
		if Player then 
			...
			if Decal and Decal:FindFirstChildOfClass'Decal' then
				if writefile then  
					...
					notif("Decal has been stolen.","Check your exploits workspace folder",5,nil)
				else
					print("Decal From: "..Player.Name.." Id: "..tostring(string.match(Decal.Decal.Texture,'%d+')))
					notif("Decal has been stolen.","It has been printed to your console (F9) due to your exploit not supporting writefile",5,nil)
				end 
			end
		end
	end
...
function(args, speaker)
	if args[1] then 
		...
		if args[1] == "sawed" then args[1] = "sawed off" end
		execCmd('noclip')
		local workspaceChildren = workspace:GetDescendants()
		for i = 1,#workspaceChildren do 
			local Child = workspaceChildren[i]
...
function(args, speaker)
	...
	connection = game.Players.LocalPlayer:GetMouse().KeyDown:connect(function(key)
		...
		state = not state
		if state then
			bforce.Parent = char.Torso
			bav.Parent = char.Torso
			bav.AngularVelocity = char.Torso.CFrame.rightVector * -25
		else
			bav.Parent = nil
			bforce.Parent = nil
		end
	...
	connection = game.Players.LocalPlayer:GetMouse().KeyDown:connect(function(key)
		...
		if state then
			bav.Parent = char.Torso
			bav.AngularVelocity = char.Torso.CFrame.upVector * -50
		else
			bav.Parent = nil
		end
	end)
...
function(args, speaker)
	orbital = not orbital
	local orbiting = PlrFinder(args[1])
	if orbital then
		local workspaceChildren = workspace:GetDescendants()
		for i = 1,#workspaceChildren do 
			local Child = workspaceChildren[i]
		...
		o.Parent = speaker.Character.HumanoidRootPart
		o.Name = "Orbit"
		o.Target = orbiting.Character.HumanoidRootPart
		o:Fire()
		o.MaxThrust = 5000
		o.MaxSpeed = 100
		o.ThrustP = 5000
		execCmd('noclip')
		speaker.Character.Humanoid:ChangeState(16)
		speaker.Character.Head.CanCollide = false
		speaker.Character.Torso.CanCollide = false
		speaker.Character["Left Leg"].CanCollide = false
		speaker.Character["Right Leg"].CanCollide = false

		local RunService = game:GetService("RunService")
		local Connection = RunService.Heartbeat:Connect(function()
			if orbiting.Character.Humanoid.Health == 0 then
				execCmd('clip')
				game.Players.LocalPlayer.Character.HumanoidRootPart.Orbit:Destroy()
...
addcmd('kneel',{},
function(args, speaker)
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.walk:GetChildren()) do v.AnimationId = 'rbxassetid://180426354' end
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.jump:GetChildren()) do v.AnimationId = 'rbxassetid://125750702' end
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.idle:GetChildren()) do v.AnimationId = 'rbxassetid://287325678' end
end)
...
function(args, speaker)
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.walk:GetChildren()) do v.AnimationId = 'rbxassetid://180426354' end
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.jump:GetChildren()) do v.AnimationId = 'rbxassetid://125750702' end
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.idle:GetChildren()) do v.AnimationId = 'rbxassetid://180435571' end
end)

addcmd('ghost',{},
function(args, speaker)
	game.Players.LocalPlayer.PlayerGui.LocalScript.Run.AnimationId = 'rbxassetid://451687057'
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.walk:GetChildren()) do v.AnimationId = "rbxassetid://451687057" end
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.jump:GetChildren()) do v.AnimationId = 'rbxassetid://451686028' end
	for _,v in pairs(game.Players.LocalPlayer.Character.Animate.idle:GetChildren()) do v.AnimationId = 'rbxassetid://451686028' end
end)

addcmd('find',{},
function(args, speaker)
	local found = PlrFinder(args[1])
	local bgui = Instance.new("BillboardGui", found.Character.Head)
	bgui.AlwaysOnTop = true
	bgui.Adornee = found.Character.Head
	bgui.Enabled = true
...
addcmd('unfind',{},
function(args, speaker)
	local unfound = PlrFinder(args[1])
	unfound.Character.Head.BillboardGui:Destroy()
end)
...
function(args, speaker)
	flingz = not flingz
	local flinging = PlrFinder(args[1])
	if flingz then
		local workspaceChildren = workspace:GetDescendants()
		for i = 1,#workspaceChildren do 
			local Child = workspaceChildren[i]
		...
		execCmd('noclip')
		Teleport(flinging.Character.Head.CFrame)
		wait(3)
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players) do
			if viewDied then
		...
		a.Parent = speaker.Character.HumanoidRootPart
		a.Name = "Annoy"
		a.Target = flinging.Character["Head"]
		a:Fire()
		a.MaxThrust = 5000000
		a.MaxSpeed = 1000000
		a.ThrustP = 5000000
		...
		local RunService = game:GetService("RunService")
		local Connection = RunService.Heartbeat:Connect(function()
			if flinging.Character.Humanoid.Health == 0 or speaker.Character.Humanoid.Health < 5 or speaker.Character.KO.Value < 3 then
				execCmd('clip')
				speaker.Character.HumanoidRootPart.Annoy:Destroy()
				speaker.Character:BreakJoints()
				wait(5.4)
				execCmd('noclip')
				Teleport(flinging.Character.Head.CFrame)
	...
	local Connection = RunService.Heartbeat:Connect(function()
		local player = game.Players.LocalPlayer
		local MAX_RANGE = 150 --change here
		if speaker:DistanceFromCharacter(flinging.Character.Head.Position) > MAX_RANGE then
			execCmd('clip')
			speaker.Character.HumanoidRootPart.Annoy:Destroy()
			speaker.Character:BreakJoints()
			wait(5.4)
...
addcmd('unannoy',{'unann'},
function(args, speaker)
	execCmd('clip')
	game.Players.LocalPlayer.Character.Humanoid:ChangeState(14)
	game.Players.LocalPlayer.Character.HumanoidRootPart.Annoy:Destroy()
	execCmd('unview')
	local CoreGuiDescendants = CoreGui:GetDescendants() 
	for i = 1,#CoreGuiDescendants do 
		local Child = CoreGuiDescendants[i]
...
addcmd('instareset',{'ins'},
function(args, speaker)
	instareset = true
	notif("InstantReset","Has been set to True")
	repeat wait()
		if game.Players.LocalPlayer.Character.Humanoid.Health < 5 or game.Players.LocalPlayer.Character.KO.Value < 3 then
			game.Players.LocalPlayer.Character:BreakJoints()
			wait(5)
		end
	until instareset == false
end)

addcmd('noinstantreset',{'noins'},
function(args, speaker)
	instareset = false
	notif("InstantReset","Has been set to False")
end)
